...
//	private int POSITION_TRAIL = POSITION_RIGHT;
	 ...
	 * @param usedStyle
	 */
	public BorderInfo resolveTableLeftBorder( IStyle tableLeft, IStyle rowLeft,
	 ...
	 * @param usedStyle
	 */
	public BorderInfo resolveTableTopBorder( IStyle tableTop, IStyle rowTop,
			IStyle columnTop, IStyle cellTop )
	{
		return resolveBorder( tableTopBorderCache, new BorderStyleInfo[]{
	 ...
	 * @param usedStyle
	 */
	public BorderInfo resolveTableBottomBorder( IStyle tableBottom,
			IStyle rowBottom, IStyle columnBottom, IStyle cellBottom )
	{

		return resolveBorder( tableBottomBorderCache, new BorderStyleInfo[]{
	...
	public BorderInfo resolvePagenatedTableTopBorder( IStyle rowTop,
			IStyle cellTop )
	{
		return resolveBorder( pagenatedTableTopBorderCache,
				new BorderStyleInfo[]{
						new BorderStyleInfo( cellTop, POSITION_TOP ),
						new BorderStyleInfo( rowTop, POSITION_TOP )} );
	}
	...
	public BorderInfo resolvePagenatedTableBottomBorder( IStyle rowBottom,
			IStyle cellBottom, IStyle usedStyle )
	{
		return resolveBorder( pagenatedTableBottomBorderCache,
				new BorderStyleInfo[]{
						new BorderStyleInfo( cellBottom, POSITION_BOTTOM ),
						new BorderStyleInfo( rowBottom, POSITION_BOTTOM )} );
	}
	 ...
	 * @param usedStyle
	 */
	public BorderInfo resolveTableRightBorder( IStyle tableRight,
	 ...
	 * @param usedStyle
	 */
	public BorderInfo resolveCellLeftBorder( IStyle preColumnRight,
	 ...
	 * @param usedStyle
	 */
	public BorderInfo resolveCellTopBorder( IStyle preRowBottom, IStyle rowTop,
			IStyle preCellBottom, IStyle cellTop )
	{
		return resolveBorder( cellTopBorderCache, new BorderStyleInfo[]{
	...
	private BorderInfo resolveBorder( BorderCache cache,
			BorderStyleInfo[] styles )
	{
		...
		if ( maxWidth == 0 || maxCount == 1 )
		{
			...
			int width = PropertyUtil
					.getDimensionValue( borderWidths[maxFirstIndex] );
			if ( width > 0 )
			{
				border = new BorderInfo( color, borderStyles[maxFirstIndex],
						borderWidths[maxFirstIndex] );
			}
		}
		else
		{
			...
			int width = PropertyUtil
					.getDimensionValue( borderWidths[maxStyleIndex] );
			if ( width > 0 )
			{
				border = new BorderInfo( color, borderStyles[maxStyleIndex],
						borderWidths[maxStyleIndex] );
			}
		}
