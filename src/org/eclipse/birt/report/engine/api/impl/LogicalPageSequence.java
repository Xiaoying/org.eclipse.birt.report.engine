...
public class LogicalPageSequence
{
	...
	public long[][] getPhysicalPageNumbers( long[][] logicalPages )
	{
		...
		for ( int i = 0; i < visiblePages.length; i++ )
		{
			...
			long lastLogicalPage = logicalPageNumber + lastPhysicalPage
					- firstPhysicalPage + 1;
			...
			while ( j < logicalPages.length )
			{
				...
				long firstPage = firstPhysicalPage
						+ Math.max( firstLogicalPage, range[0] )
						- firstLogicalPage;
				long lastPage = firstPhysicalPage
						+ Math.min( lastLogicalPage, range[1] )
						- firstLogicalPage;
