...
public class HTMLEncodeUtil extends XMLEncodeUtil
{
	...
	static public String encodeText( String text, boolean whitespace )
	{
		...
		int index = testEscape( chars, whitespace
				? HTML_WHITE_SPACE_ENCODING
				: HTML_TEXT_ENCODING );
		...
		while ( index < length )
		{
			...
			{
				logger.log( Level.WARNING, MESSAGE_INVALID_CHARACTER, Integer
						.valueOf( c ) );
			}
		}
	 ...
	 * 
	 * For single white space, if it is the first character or after a BR,
	 * replace it to &nbsp;. If it is the last character or followed by a <BR>
	 * replace it to &nbsp; otherwise, keep it unchanged. For multiple white
	 * space, replace it to &nbsp; except the last one. If it is the last one or
	 * followned by a <BR>
	 * , replace all the whitespaced to BR
	 */
	static public int encodeWhitespace( char c1, char[] chars, int offset,
			StringBuilder sb )
	{
	...
	static int encodeLineBreak( char c, char[] chars, int offset,
			StringBuilder sb )
	{
		int index = offset;
