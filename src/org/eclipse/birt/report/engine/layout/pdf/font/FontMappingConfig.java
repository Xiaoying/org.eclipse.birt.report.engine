...
public class FontMappingConfig
{
	...
	public void merge( FontMappingConfig config )
	{
		...
		while ( iter.hasNext( ) )
		{
			...
			CompositeFontConfig newConfig = (CompositeFontConfig) entry
					.getValue( );
			CompositeFontConfig oldConfig = (CompositeFontConfig) compositeFonts
					.get( fontName );
			if ( oldConfig != null )
			{
			...
			{
				compositeFonts.put( fontName, new CompositeFontConfig(
						newConfig ) );
			}
		}
	}
