...
public class FontConfigHandler extends DefaultHandler
{
	...
	private class FontState extends ParseState
	{
		...
		public ParseState startElement( String tagName )
		{
			...
			if ( TAG_FONT_ALIASES.equals( tagValue ) ||
					TAG_FONT_MAPPINGS.equals( tagValue ) )
			{
	...
	private class AllFontState extends ParseState
	{
		...
		void addComponent( String fontName, int start, int end,
				String fontFamily )
		{
		...
		CompositeFontConfig createCompositeFont( String name, Map fonts )
		{
			...
			while ( iter.hasNext( ) )
			{
				...
				CharSegment[] segment = (CharSegment[]) charSegs
						.toArray( new CharSegment[]{} );
		...
		public void end( )
		{
			...
			while ( iter.hasNext( ) )
			{
				...
				CompositeFontConfig fontConfig = createCompositeFont( fontName,
						fonts );
	...
	private class CompositeFontState extends ParseState
	{
		...
		private CompositeFontConfig createCompositeFont( )
		{
			if ( fontName != null )
			{
				CompositeFontConfig fontConfig = new CompositeFontConfig(
						fontName );
				...
				{
					CharSegment[] seg = (CharSegment[]) characters
							.toArray( new CharSegment[]{} );
				...
				while ( iter.hasNext( ) )
				{
					...
					CharSegment[] seg = (CharSegment[]) blocks
							.toArray( new CharSegment[]{} );
		...
		private class BlockState extends ParseState
		{
			...
			public void parseAttrs( Attributes attrs )
			{
				...
				if ( fontFamily != null )
				{
					...
					{
						int rangeStart = getHexValue( attrs, ATTR_RANGE_START,
								-1 );
